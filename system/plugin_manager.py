...
class YamlPluginManagerSingleton(PluginManager):
    ...
    def reloadPlugin(self, name):
        plugin_module_name_template = NormalizePluginNameForModuleName(
            "yapsy_loaded_plugin_" + name
        ) + "_%d"

        for plugin_name_suffix in range(len(sys.modules)):
            plugin_module_name = plugin_module_name_template \
                % plugin_name_suffix
            if plugin_module_name in sys.modules:
                log.warn("Reloading: %s" % plugin_module_name)
                log.warn(sys.modules)
                reload(sys.modules.get(plugin_module_name))
                return True
    ...
    def reloadPlugin(self, name):
        """
        Load a single plugin by name, or reload it if already loaded.

        This is a hack. Try not to use it.
        """
        ...
        for candidate_infofile, candidate_filepath, plugin_info \
                in self._candidates:
            ...
            plugin_module_name_template = NormalizePluginNameForModuleName(
                "yapsy_loaded_plugin_" + plugin_info.name
            ) + "_%d"
            ...
            try:
                ...
                log.error(
                    "Unable to import plugin: %s" % candidate_filepath,
                    exc_info=exc_info
                )
    ...
    def loadPlugins(self, callback=None):
        ...
        This has been modified to not delete plugin candidates when it's done.
        """
